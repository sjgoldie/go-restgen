...
- **Query timeout**: `datastore.NewPostgres(dsn, datastore.WithTimeout(10*time.Second))` (also on `NewSQLite` and the `WithDB` variants); defaults 30s PostgreSQL, 5s SQLite
- **Ownership is per method and re-enforced on update**: each `AuthConfig`'s ownership applies to its own methods; PUT/PATCH cannot change owner fields without a bypass scope
